<template>
	...
	<cell :keep-scroll-position="keepScrollPosition">
		<!-- #endif -->
		<view :class="{ 'comment-item--disabled': disabled }" :style="{'background-color':customStyle.backgroundColor}"
			:hover-class="(!clickable && !link) || disabled || showSwitch ? '' : 'comment-item--hover'"
			class="comment-item" @click="onClick">
			...
			<view class="comment-item__container"
				...
				<slot name="header">
					<view class="comment-item__header">
						<view v-if="thumb" class="comment-item__icon">
							<image :src="thumb" class="comment-item__icon-img" :class="['uni-list--' + thumbSize]" />
						</view>
						<view v-else-if="showExtraIcon" class="comment-item__icon">
				...
				<slot name="body">
					<view class="comment-item__content"
						:class="{ 'comment-item__content--center': thumb || showExtraIcon || showBadge || showSwitch }">
				...
				<slot name="footer">
					<view v-if="rightText || showBadge || showSwitch" class="comment-item__extra"
						:class="{ 'flex--justify': direction === 'column' }">
						<text v-if="rightText" class="comment-item__extra-text">{{ rightText }}</text>
...
<script>
    import commentSubItem from '@/pages/tab-bar/forum/components/comment/comment-sub-item.vue'
	/**
	 * ListItem 列表子组件
	 * @description 列表子组件
